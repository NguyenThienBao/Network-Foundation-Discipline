...
//#endregion
//#region node_modules/usehooks-ts/dist/index.js
...
function useBoolean(defaultValue = false) {
	...
	return {
		value,
		setValue,
		setTrue: (0, import_react.useCallback)(() => {
			setValue(true);
		}, []),
		setFalse: (0, import_react.useCallback)(() => {
			setValue(false);
		}, []),
...
function useCounter(initialValue) {
	...
	return {
		count,
		increment: (0, import_react.useCallback)(() => {
			setCount((x) => x + 1);
		}, []),
...
function useCountdown({ countStart, countStop = 0, intervalMs = 1e3, isIncrement = false }) {
	...
	useInterval((0, import_react.useCallback)(() => {
		if (count === countStop) {
			stopCountdown();
			return;
		}
...
function useReadLocalStorage(key, options = {}) {
	...
	if (IS_SERVER3) initializeWithValue = false;
	const deserializer = (0, import_react.useCallback)((value) => {
		if (options.deserializer) return options.deserializer(value);
		if (value === "undefined") return;
		let parsed;
...
function useScreen(options = {}) {
	let { initializeWithValue = true } = options;
	if (IS_SERVER4) initializeWithValue = false;
	const readScreen = () => {
		if (IS_SERVER4) return;
...
function getScriptNode(src) {
	...
	return {
		node,
		status: node == null ? void 0 : node.getAttribute("data-status")
	};
}
function useScript(src, options) {
	const [status, setStatus] = (0, import_react.useState)(() => {
		if (!src || (options == null ? void 0 : options.shouldPreventLoad)) return "idle";
		if (typeof window === "undefined") return "loading";
		return cachedScriptStatuses.get(src) ?? "loading";
	});
	(0, import_react.useEffect)(() => {
		if (!src || (options == null ? void 0 : options.shouldPreventLoad)) return;
		...
		if (!scriptNode) {
			...
			if (options == null ? void 0 : options.id) scriptNode.id = options.id;
...
function useSessionStorage(key, initialValue, options = {}) {
	...
	const readValue = (0, import_react.useCallback)(() => {
		const initialValueToUse = initialValue instanceof Function ? initialValue() : initialValue;
		if (IS_SERVER6) return initialValueToUse;
		try {
	...
	const setValue = useEventCallback((value) => {
		if (IS_SERVER6) console.warn(`Tried setting sessionStorage key \u201C${key}\u201D even though environment is not a client`);
		try {
			const newValue = value instanceof Function ? value(readValue()) : value;
	...
	const removeValue = useEventCallback(() => {
		if (IS_SERVER6) console.warn(`Tried removing sessionStorage key \u201C${key}\u201D even though environment is not a client`);
		const defaultValue = initialValue instanceof Function ? initialValue() : initialValue;
...
function useStep(maxStep) {
	...
	const setStep = (0, import_react.useCallback)((step) => {
	...
	return [currentStep, {
		goToNextStep: (0, import_react.useCallback)(() => {
			if (canGoToNextStep) setCurrentStep((step) => step + 1);
		}, [canGoToNextStep]),
		goToPrevStep: (0, import_react.useCallback)(() => {
